<?php
return [
    'no-products-in-cart'=>'لايوجد منتجات في السلة',
    'api-success-message'=>'تم التنفيذ بنجاح',
    'api-error-message'=>'يرجي إعادة المحاولة مرة اخري',
    'please-check-last-password'=>'تأكد من كلمة المرور القديمة',
    'product-deleted'=>'تم حذف المنتج',
    'product-not-exists'=>'المنتج غير موجود',
    'product-in-cart'=>'المنتج موجود في السلة بالفعل',
    'can-not-add-product-to-diffrent-deliver'=>'لايمكن اضافة المنتج مع منتجات السلة في حالة توصيل مختلفة',
    'must-mini-company'=>'يجب الآلتزام بالحد الأدني للشركة',

];